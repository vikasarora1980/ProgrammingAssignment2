...
makeCacheMatrix <- function(x = matrix()) {
        ...
        list(set = set, get = get, #List values and their inverse
             setinv = setinv,
             getinv = getinv)
}
...
cacheSolve <- function(x, ...) {
        inv_x <- x$getinv()  #Invoke getinv function
        if(!is.null(inv_x)) { #Check whether existing value of inverse available
                message("getting cached data") #Inverse available in Cache
                return(inv_x) #Return inverse from Cache
        }
        data <- x$get()  #Invoke value of matrix in data variable
        inv_x <- solve(data, ...) # Calculate Inverse of matrix from data variable
        x$setinv(inv_x) #Set value of inverse of matrix 
        inv_x #Return Inverse
}